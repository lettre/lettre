...
use smtp::client::net::DEFAULT_TLS_PROTOCOL;
...
impl SmtpClient {
    ...
    pub fn new_simple(domain: &str) -> Result<SmtpClient, Error> {
        ...
        tls_builder.min_protocol_version(Some(DEFAULT_TLS_PROTOCOL));

        let tls_parameters =
            ClientTlsParameters::new(domain.to_string(), tls_builder.build()?);
